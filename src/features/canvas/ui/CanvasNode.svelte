...
<div
  class="absolute select-none shadow-sm cursor-pointer"
  ...
  {#if node.type === 'goroutine'}
    <div class="w-[145px] rounded-xl border {isInvalidTarget ? 'border-red-500 bg-red-950/40 shadow-[0_0_0_3px_rgba(239,68,68,0.4)] animate-pulse' : isConnectSource ? 'border-rose-500 shadow-[0_0_0_3px_rgba(244,63,94,0.25)]' : isSelected ? 'border-emerald-500/60 shadow-[0_0_0_2px_rgba(16,185,129,0.2)]' : 'border-zinc-700'} bg-zinc-900/95 backdrop-blur-sm p-3 transition-colors hover:border-zinc-600">
      ...
      <div class="flex items-center justify-between text-[10px] font-mono">
        ...
        <span class="px-2 py-0.5 rounded-full {node.status === '_Grunning' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' : node.status === '_Gwaiting' ? 'bg-amber-500/15 text-amber-400 border-amber-500/30' : 'bg-cyan-500/15 text-cyan-400 border-cyan-500/30'} border">
