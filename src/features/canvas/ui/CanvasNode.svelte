<script lang="ts">
  ...
  import type { CanvasNode, ChannelNode } from '$shared/types/nodes';
  ...
  let { node, isSelected, isConnectSource, isValidTarget, isInvalidTarget, onPointerDown, onPointerUp }: Props = $props();
...
<div
  ...
  tabindex="0"
>
  <div class="w-[145px] rounded-xl border {cardStyle} bg-zinc-900/95 backdrop-blur-md p-3 font-mono transition-[border-color,box-shadow,background-color] duration-150">
    <div class="flex items-center gap-2 mb-2">
      <div class="w-6 h-6 rounded-lg flex items-center justify-center shrink-0 {isGoroutine ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20'}">
        {#if isGoroutine}
    ...
    {#if isGoroutine}
      <div class="flex items-center justify-between text-[10px]">
        ...
        <span class="px-2 py-0.5 rounded-full border bg-emerald-500/15 text-emerald-400 border-emerald-500/30 font-bold">
      ...
      {#if ch.capacity <= 6}
        <div class="flex items-center gap-1 mt-1">
          {#each Array(Math.max(ch.capacity, 1)) as _, i}
            <div class="h-1.5 flex-1 rounded-full transition-colors duration-150 {i < ch.values.length ? 'bg-cyan-400 shadow-[0_0_6px_rgba(6,182,212,0.6)]' : 'bg-zinc-800 border border-zinc-700/50'}"></div>
          {/each}
        </div>
      {:else}
        <div class="space-y-1 mt-1">
          <div class="w-full bg-zinc-800 h-1.5 rounded-full overflow-hidden border border-zinc-700/50">
            <div
