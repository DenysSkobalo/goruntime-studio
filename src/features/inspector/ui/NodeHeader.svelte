<script lang="ts">
  ...
  let doc = $derived(docId ? RUNTIME_DOCS.find((d) => d.id === docId) : undefined);
...
<div class="flex items-center justify-between border-b border-zinc-800 pb-2.5 mb-3 gap-2 w-full">
  ...
  {#if doc}
    <div class="flex items-center gap-1.5 shrink-0">
      ...
      <button
        onclick={() => openPlatformDocs(doc!.id)}
        class="p-1 text-zinc-400 hover:text-sky-400 rounded transition"
