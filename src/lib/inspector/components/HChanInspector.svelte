<script lang="ts">
  ...
  import { Lock, LockOpen, Database, ArrowDown, ArrowUp, CircleAlert } from 'lucide-svelte';
    ...
    {#each channels as hchan (hchan.address)}
      ...
      <div class="glow-card p-6 animate-fade-in">
        ...
        <div class="mb-6 grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
          ...
          <div class="inner-card p-3">
            <span class="text-zinc-500 dark:text-zinc-400 block mb-1">{i18n.t('inspector.hchan.dataqsiz')}</span>
            <span class="text-lg font-bold text-zinc-900 dark:text-white">{hchan.dataqsiz}</span>
          </div>
          <div class="inner-card p-3">
